...
resource "aws_sqs_queue" "cart_ping_queue" {
  ...
  redrive_policy            = "{\"deadLetterTargetArn\":\"${aws_sqs_queue.cart_ping_dead_letter_queue.arn}\",\"maxReceiveCount\":4}"
}
...
resource "aws_lambda_event_source_mapping" "event_source_mapping" {
  batch_size        = 1
  event_source_arn  = aws_sqs_queue.cart_ping_queue.arn
  enabled           = true
  function_name     = aws_lambda_function.cart_ping.arn
}

resource "aws_cloudwatch_log_group" "log_group" {
  name              = "/aws/lambda/${aws_lambda_function.cart_ping.function_name}"
  retention_in_days = 7
}