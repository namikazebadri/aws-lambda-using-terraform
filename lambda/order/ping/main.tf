...
resource "aws_sqs_queue" "order_ping_dead_letter_queue" {
  name                        = "${var.LAMBDA_ENV}-${local.project}_${local.module}_${local.function}-DLQ"
  message_retention_seconds   = 60
  visibility_timeout_seconds  = 60
}

resource "aws_sqs_queue" "order_ping_queue" {
  name                      = "${var.LAMBDA_ENV}-${local.project}_${local.module}_${local.function}"
  delay_seconds             = 90
  max_message_size          = 2048
  message_retention_seconds = 86400
  receive_wait_time_seconds = 10
  redrive_policy            = "{\"deadLetterTargetArn\":\"${aws_sqs_queue.order_ping_dead_letter_queue.arn}\",\"maxReceiveCount\":4}"
}

resource "aws_lambda_function" "order_ping" {
...
resource "aws_lambda_event_source_mapping" "event_source_mapping" {
  batch_size        = 1
  event_source_arn  = aws_sqs_queue.order_ping_queue.arn
  enabled           = true
  function_name     = aws_lambda_function.order_ping.arn
}
